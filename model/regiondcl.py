...
class PatternEncoder(nn.Module):
    def __init__(self, d_building, d_poi,d_svi, d_hidden, d_feedforward,
        ...
        self.poi_projector = ProjectionHead(d_poi, d_hidden)
    ...
    def get_all(self, building_feature, building_mask, xy, poi_feature, poi_mask, svi_emb, svi_mask):
        ...
        building_score = self.building_gate(building_feature, 1) #(seq, b, 1)
        ...
        # ==========>
        building_encoding = (building_encoding * building_score) #avg pooling
        encoding_list = [building_encoding.mean(dim=0)]
        ...
        if poi_feature is not None:
            ...
            encoding_list.append(poi_encoding.mean(dim=0)) #(batch_size,d)
        ...
        if self.use_svi and svi_emb is not None:
            ...
            encoding_list.append(svi_emb.mean(dim=0))
            # mask_list.append(svi_mask)
        # MoE
        gate_score = self.gates(torch.cat(encoding_list, dim=1)) #(b, 3*d)
